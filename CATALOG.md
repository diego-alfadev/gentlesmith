...
| Reference | Profile knows when to suggest/use an installed/external skill through a compact local `references/<slug>` fragment. | External `think-through`-style skill for uncertain work |
...
Keep `skills:` as simple metadata/package list for now. Do not introduce structured skill objects until the reference model proves insufficient. Do not vendor third-party skills as built-ins; reference/install them from their upstream source.
